...
class O2APIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        try:
            ...
            params = {
                "market_id": market_id,
                "precision": CONSTANTS.REST_DEFAULT_PRECISION  # Default 10
            }
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        try:
            ...
            for trading_pair in self._trading_pairs:
                ...
                depth_view_subscription = {
                    "action": CONSTANTS.WS_SUBSCRIBE_DEPTH_VIEW,
                    "market_id": market_id,
                    "precision": CONSTANTS.WS_DEFAULT_PRECISION,
                    "frequency": CONSTANTS.WS_PERIODIC_DATA_FREQUENCY
                }
                ...
                depth_view_request = WSJSONRequest(payload=depth_view_subscription)
                trade_request = WSJSONRequest(payload=trade_subscription)

                subscription_tasks.extend([
                    ws.send(depth_view_request),
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        self.logger().info("Successfully connected to O2 WebSocket")
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        try:
            ...
            snapshot_msg: OrderBookMessage = O2OrderBook.snapshot_rest_message_from_exchange(
                snapshot_response,
                snapshot_timestamp,
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        try:
            ...
            # Compatible with both rest and websocket formats
    ...
    async def _parse_order_book_snapshot_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        try:
            if raw_message.get("action") != CONSTANTS.WS_SUBSCRIBE_DEPTH_VIEW:
                return

            raw_market_id = raw_message.get("market_id", "")
            market_id = o2_utils.normalize_market_id(raw_market_id)
            if not market_id:
                self.logger().warning("No market_id in O2 snapshot message")
                return

            market_id_map = await self._connector.trading_pair_market_id_map()
            if market_id not in market_id_map.inverse:
                self.logger().warning(f"Unknown market_id in O2 snapshot message: {market_id}")
                return

            trading_pair = market_id_map.inverse[market_id]
            snapshot_timestamp: float = time.time()

            snapshot_message: OrderBookMessage = O2OrderBook.snapshot_ws_message_from_exchange(
                raw_message,
                snapshot_timestamp,
                metadata={"trading_pair": trading_pair, "connector": self._connector}
            )
            ...
            self.logger().debug(f"Processed O2 snapshot message for {trading_pair}")
        except Exception as e:
            self.logger().error(f"Error parsing O2 snapshot message: {e}", exc_info=True)

    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        """
        O2 uses snapshot-only protocol. This method should not be called since
        all depth messages are routed directly to the snapshot queue.
        """
        return
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        if action == CONSTANTS.WS_SUBSCRIBE_DEPTH_VIEW:
            return self._snapshot_messages_queue_key
        elif action == CONSTANTS.WS_SUBSCRIBE_TRADES:
            return self._trade_messages_queue_key
        else:
            self.logger().warning(f"Unknown O2 WebSocket action '{action}', routing to snapshots")
            return self._snapshot_messages_queue_key

    def _get_messages_queue_keys(self) -> List[str]:
        return [self._trade_messages_queue_key, self._snapshot_messages_queue_key]
